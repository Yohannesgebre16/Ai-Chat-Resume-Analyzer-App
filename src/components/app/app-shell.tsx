          ...
          <div className="flex min-w-0 items-center gap-2">
            {tab === 'chat' && activeSession ? (
              ...
              <span className="flex items-center gap-1.5 text-sm font-medium">
                <Sparkles className="h-4 w-4 text-emerald-500" /> Zai Chat
              </span>
            )}
          </div>
        </header>
          ...
          {showChat && activeSessionId ? (
            ...
            <div className="flex flex-1 flex-col items-center justify-center px-4 text-center">
              ...
              <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                Create a chat to ask anything. Edit prompts or regenerate replies to branch the
                thread and explore different answers.
              </p>
              <Button className="mt-5" onClick={handleNew} disabled={createSession.isPending}>
                New chat
              </Button>
            </div>
          )}
